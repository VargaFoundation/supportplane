...
import varga.supportplane.infra.MetricIngestService;
    ...
    public void parseBundle(Bundle bundle) {
        try {
            ...
            if (bundle.getCluster() != null && !clusterMetadata.isEmpty()) {
                ...
                // Archive snapshot for AI time-series analysis
                metricHistoryRepository.save(MetricHistory.builder()
                        .cluster(cluster)
                        .snapshotData(new HashMap<>(clusterMetadata))
                        .build());

                // Ingest decomposed metrics into ClickHouse + logs into Loki
                metricIngestService.ingest(cluster.getId(), cluster.getClusterId(), clusterMetadata);
            }
